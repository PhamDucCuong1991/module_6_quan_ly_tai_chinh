...
List<Object> getSumByTypeAndDate(LocalDate startDate,LocalDate endDate,Long userId);
    @Query(value = "select c from Cash  c where c.category.id=:categoryId")
    List<Cash> findCashByCategory_Id(@Param("categoryId")Long categoryId);

    @Query(value = "select month(c.date),sum(c.money),c.type from Cash  c where c.account.id=:userId and year(c.date)=:year group by month(c.date),year(c.date),c.type")
    List<Object> getDifferenceIncomeExpenseByMonth(@Param("userId") Long userId,@Param("year")int year);

    @Query(value = "select sum(c.money),c.wallet.name from Cash  c where c.account.id=:userId and c.date=:date and c.type = 'expence'group by c.wallet.name order by sum(c.money) desc")
